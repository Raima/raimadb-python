...
class SetTest(unittest.TestCase):
    ...
    def verify_rows_forward_from_current_position(self, cursor:RdmCursor, columns:list, expected:list):
    ...
    def verify_rows (self, cursor:RdmCursor, columns:list, expected:list):
        ...
        self.assertEqual(status, Status.Okay, "Failed to move to before first row")
        self.verify_rows_forward_from_current_position(cursor, columns, expected)
        status = trans.end()
        self.assertEqual(status, Status.Okay, "Failed to end transaction")
    ...
    def test_get_sibling_rows_of_first_member(self):
        ...
        self.verify_rows_forward_from_current_position(siblings, ["ID", "OWNER_ID", "STR"], [
            (1, 1, "member1"),
            (2, 1, "member2"),
            (3, 1, "member3")
        ])
    ...
    def test_get_sibling_rows_at_positionof_of_first_member(self):
        self.insert_test_data()
        status, trans = self.db.startRead()
        self.assertEqual(status, Status.Okay, "Failed to start read")
        status, members = self.db.getRows("MEMBER")
        ...
        status = members.moveToFirst()
        self.assertEqual(status, Status.Okay, "Failed to move to first member")
        status, siblings = members.getSiblingRowsAtPosition("MEMBER_OWNER_ID")
        self.assertEqual(status, Status.Okay, "Failed to get siblings of first member")
        self.verify_rows_forward_from_current_position(siblings, ["ID", "OWNER_ID", "STR"], [
            (2, 1, "member2"),
            (3, 1, "member3")
